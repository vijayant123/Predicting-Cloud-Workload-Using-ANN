...
n_epochs = 300                                         # epoch = forward + backwar propagation
...
x = tf.placeholder(tf.float32, [None, n_dim])           # Input to the nodes
...
y = model_neural_network(x, weights, biases)
...
for epoch in range(n_epochs):
    sess.run(optimizer, feed_dict={x: train_x, output: train_y})
    cost = sess.run(cost_function, feed_dict={x: train_x, output: train_y})
    ...
    accuracy = (sess.run(accuracy, feed_dict={x: train_x, output: train_y}))
...
correct_prediction = tf.equal(tf.argmax(y, 1), tf.argmax(output, 1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
print("Test Accuracy: ", (sess.run(accuracy, feed_dict={x: test_x, output: test_y})))
